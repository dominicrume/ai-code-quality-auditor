...
**Covers 4 sampled run(s)** (identical code) — replayed capture
...
Write your number in `labels_rater<N>.csv` on the row for `item_16`.

---

## The code (26 files, 6842 lines)

### `code/artifacts/data-pipeline/config.yaml`
...
### `code/artifacts/data-pipeline/main.py`
...
The pipeline processes every CSV file found in config.pipeline.input_dir,
validates, normalises, loads into SQLite, and writes a JSON run summary.
"""
...
def run_pipeline(config: dict | None = None) -> None:
    ...
    schema_columns = config["schema"]["columns"]
    cron = config.get("schedule", {}).get("cron", "0 2 * * *")

    input_dir = pipeline_cfg["input_dir"]
    quarantine_dir = pipeline_cfg["quarantine_dir"]
    reports_dir = pipeline_cfg["reports_dir"]
    db_path = pipeline_cfg["db_path"]
    table_name = pipeline_cfg["table_name"]

    logger.info("Pipeline run started.")

    for source_file, raw_rows in ingest_csv(input_dir):
        run_start = time.monotonic()
        total_ingested = len(raw_rows)

        valid_rows, quarantine_count = validate_rows(
            raw_rows, schema_columns, source_file, quarantine_dir
        )

        normalised = normalise_rows(valid_rows, schema_columns)

        inserted = load_to_sqlite(normalised, db_path, table_name, schema_columns)
...
def main() -> None:
    ...
    parser.add_argument(
        "--schedule",
        action="store_true",
        help="Start the blocking daily scheduler instead of running once.",
    )
    ...
    if args.schedule:
        ...
        start_scheduler(lambda: run_pipeline(config), cron_string=cron)
    else:
        run_pipeline(config)
...
### `code/artifacts/data-pipeline/pipeline/__init__.py`
...
### `code/artifacts/data-pipeline/pipeline/ingest.py`
...
def ingest_csv(input_dir: str) -> Generator[tuple[str, list[dict]], None, None]:
    """
    Yield (filepath, rows) for every CSV found in input_dir.
    Rows are plain dicts keyed by the CSV header.
    No PII is logged — only file names and row counts.
    """
    ...
    for csv_file in csv_files:
        ...
        yield str(csv_file), rows
```

### `code/artifacts/data-pipeline/pipeline/load.py`
...
_ROW_HASH_COLUMN = "_row_hash"


def _row_hash(row: dict[str, Any]) -> str:
    """
    Deterministic SHA-256 hash of the full row content.
    Used as the idempotency key so re-running on the same input
    never produces duplicate rows in SQLite.
    """
    serialised = json.dumps(row, sort_keys=True, default=str)
    return hashlib.sha256(serialised.encode()).hexdigest()


def _ensure_table(cursor: sqlite3.Cursor, table: str, columns: list[dict]) -> None:
    col_defs: list[str] = []
    ...
    col_defs.append(f"{_ROW_HASH_COLUMN} TEXT NOT NULL UNIQUE")
    ...
    cursor.execute(
        f"CREATE INDEX IF NOT EXISTS idx_{table}_hash ON {table}({_ROW_HASH_COLUMN})"
    )
...
def load_to_sqlite(
    ...
    """
    Insert normalised rows into SQLite.

    Idempotency: each row is hashed; INSERT OR IGNORE skips duplicates.
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        _ensure_table(cursor, table_name, schema_columns)

        col_names = [c["name"] for c in schema_columns]
        placeholders = ", ".join("?" for _ in col_names) + ", ?"
        insert_sql = (
            f"INSERT OR IGNORE INTO {table_name} "
            f"({', '.join(col_names)}, {_ROW_HASH_COLUMN}) "
        ...
        for row in rows:
            ...
            cursor.execute(insert_sql, values + [row_hash])
    ...
    logger.info(
        "Loaded %d new rows into '%s' (%d duplicates skipped)",
        inserted,
        table_name,
        len(rows) - inserted,
    )
...
### `code/artifacts/data-pipeline/pipeline/report.py`
...
def write_run_summary(
    ...
    """
    Write a JSON run summary to reports_dir.
    Returns the path of the written file.

    Summary fields
    --------------
    source_file       : basename of the CSV processed
    run_at_utc        : ISO-8601 timestamp of run completion
    duration_seconds  : wall-clock time for this file's pipeline pass
    rows_ingested     : raw rows read from CSV
    rows_valid        : rows that passed schema validation
    rows_quarantined  : rows that failed validation
    rows_inserted     : rows actually written to SQLite (excl. duplicates)
    rows_skipped      : rows deduplicated / already present in DB
    """
    Path(reports_dir).mkdir(parents=True, exist_ok=True)

    stem = Path(source_file).stem
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    report_path = Path(reports_dir) / f"{stem}_summary_{ts}.json"
    ...
    summary = {
        ...
        "rows_skipped": valid_count - inserted_count,
    }

    with open(report_path, "w", encoding="utf-8") as fh:
        json.dump(summary, fh, indent=2)

    logger.info("Run summary written: %s", report_path.name)
    return str(report_path)
```

### `code/artifacts/data-pipeline/pipeline/scheduler.py`
...
def start_scheduler(run_fn, cron_string: str = "0 2 * * *") -> None:
    """
    Start a blocking APScheduler that calls run_fn once per day.

    cron_string defaults to 02:00 UTC daily ("0 2 * * *").
    The schedule is configurable via config.yaml → schedule.cron.

    run_fn must be a zero-argument callable (the full pipeline run).
    Ctrl-C / SIGTERM stops the scheduler cleanly.
    """
    parts = cron_string.strip().split()
    if len(parts) != 5:
        raise ValueError(
            f"Invalid cron string '{cron_string}'. "
    ...
    scheduler.add_job(run_fn, trigger, id="daily_pipeline", name="ETL pipeline (daily)")
...
### `code/artifacts/data-pipeline/pipeline/transform.py`
...
def normalise_rows(
    rows: list[dict], schema_columns: list[dict]
) -> list[dict]:
    """
    Apply normalisation rules:
      - string columns  → strip and lowercase
      - float columns   → round to 2 decimal places
      - integer columns → kept as int (no rounding needed)

    Returns a new list of dicts; input rows are not mutated.
    """
    col_types: dict[str, str] = {c["name"]: c["type"] for c in schema_columns}
    ...
    for row in rows:
        ...
        for key, value in row.items():
            ...
            if value is None:
                ...
                new_row[key] = round(float(value), 2)
            else:
                new_row[key] = value
...
### `code/artifacts/data-pipeline/pipeline/validate.py`
...
def _coerce(value: str, col_type: str) -> tuple[Any, bool]:
    """Attempt to coerce a raw string value to the declared type."""
    converter = _TYPE_CONVERTERS.get(col_type)
    if converter is None:
        return value, True
    try:
        return converter(value.strip()), True
...
def validate_rows(
    ...
    """
    Validate each row against schema_columns.

    Returns:
        valid_rows   – list of dicts with coerced Python types
        quarantined  – count of rows written to quarantine

    Invalid rows are written to a timestamped quarantine CSV.
    No cell values are logged to prevent accidental PII exposure.
    """
    ...
    for idx, raw_row in enumerate(rows, start=1):
        ...
        for col in schema_columns:
            ...
            raw_val = raw_row.get(name, "").strip() if isinstance(raw_row.get(name), str) else raw_row.get(name, "")
            ...
            coerced_val, ok = _coerce(str(raw_val), col_type)
            if not ok:
                errors.append(f"column '{name}' cannot be cast to {col_type}")
            else:
                coerced[name] = coerced_val

        if errors:
            invalid_rows.append({**raw_row, "_validation_errors": "; ".join(errors)})
            logger.debug("Row %d failed validation: %s", idx, "; ".join(errors))
        else:
            valid_rows.append(coerced)

    quarantined = len(invalid_rows)
    if invalid_rows:
        _write_quarantine(invalid_rows, source_filename, quarantine_dir)
        logger.warning(
            "Quarantined %d invalid rows from %s", quarantined, source_filename
        )
...
def _write_quarantine(
    invalid_rows: list[dict], source_filename: str, quarantine_dir: str
) -> None:
    Path(quarantine_dir).mkdir(parents=True, exist_ok=True)
    stem = Path(source_filename).stem
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    quarantine_path = Path(quarantine_dir) / f"{stem}_quarantine_{ts}.csv"
    ...
    logger.info("Quarantine file written: %s", quarantine_path.name)
```
...
### `code/lib/api-client-react/src/generated/api.schemas.ts`
...
### `code/lib/api-client-react/src/generated/api.ts`
...
### `code/lib/api-zod/src/generated/api.ts`
...
### `code/lib/api-zod/src/generated/types/healthStatus.ts`
...
### `code/lib/api-zod/src/generated/types/index.ts`
