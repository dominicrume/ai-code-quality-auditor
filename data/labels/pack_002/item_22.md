...
`item_22`. Answer from the code alone; do not run the tools.
...
```
"""Agent Education System — minimal Flask REST API.
...
COURSES = {
    "corp-tesco-standards": {
        ...
        "type": "corporate",
        "description": "Mock corporate training module covering Tesco store standards.",
        "lessons": [
            {
                "id": 1,
            ...
            {
                "id": 2,
                "title": "Health & Safety on the Shop Floor",
    ...
    "acad-ai-fundamentals": {
        ...
        "type": "academic",
        "description": "Mock academic module introducing core AI concepts.",
        "lessons": [
            {
                "id": 1,
            ...
            {
                "id": 3,
                "title": "Neural Networks in One Page",
...
from app import _tokens, _users, app
...
def test_view_corporate_module(client):
    _register(client)
    token = _login(client).get_json()["token"]
    r = client.get(
        "/courses/corp-tesco-standards",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert r.status_code == 200
