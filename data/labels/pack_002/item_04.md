...
`item_04`. Answer from the code alone; do not run the tools.
...
from config import DB_PATH, INPUT_DIR, TABLE_NAME
from pipeline import normalise, run, validate_row


def _write_input(rows_csv: str):
    p = INPUT_DIR / "input.csv"
    p.write_text(rows_csv, encoding="utf-8")
    return p


def test_validate_and_normalise():
    row = validate_row({"id": "1", "name": "ALICE", "email": "A@B.COM", "amount": "10.126", "category": "X"})
    n = normalise(row)
    assert n["name"] == "alice"
    assert n["amount"] == 10.13
...
def test_run_and_idempotent(tmp_path):
    if Path(DB_PATH).exists():
        Path(DB_PATH).unlink()
    _write_input(
        "id,name,email,amount,category\n"
        "1,Alice,a@b.com,10.126,Foo\n"
        "2,Bob,b@c.com,5.5,Bar\n"
        "bad,row,x,notnum,Baz\n"
    )
    s1 = run()
...
if __name__ == "__main__":
    test_validate_and_normalise()
    test_run_and_idempotent(None)
    print("ok")
```
